...
type LoggerInterface interface {
	...
	Write(level int,text string) error
	Destroy()
	Flush()
}
...
func (w *LogWriter) Write(level int,text string) error {
	if level > w.Level {
		return nil
	}
	n := 24 + len(text) // 24 stand for the length "2013/06/23 21:00:22 [T] "
	w.docheck(n)
	w.Logger.Print(text)
	return nil
}
...
func (w *LogWriter) DoRotate() error {
	...
	if err == nil { // file exists
		...
		for ; err == nil && num <= 999; num++ {
			fname = w.Filename + fmt.Sprintf(".%s.%03d", time.Now().Format(constant.FORMAT2), num)
